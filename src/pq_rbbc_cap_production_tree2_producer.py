...
RELATION_ID = "pq-rbbc/cap/production-tree-producer-index-2/v1"
TREE_INDEX = 2
LEAVES = 1 << 11
EXTENSION_DEGREE = 12
...
FROZEN_TREE_COMPONENT_SHA256 = (
    "0f1436bae35d6ad66e09375cffc4609efd62eea7f0c6253454f2657a922b3115"
)
FROZEN_ROWS = 25_666_386
FROZEN_LOCAL_WIRES = 19_478_436
FROZEN_MAX_WIRE_ID = 59_673_032
FROZEN_STREAM_BYTES = 8_961_160_817
FROZEN_STREAM_SHA256 = (
    "ad31a74cdf00ee96c646a9142da459069655e528aca3cb58cad07dc2b3b26fb8"
)
FROZEN_ASSIGNMENT_BYTES = 486_961_028
FROZEN_ASSIGNMENT_SHA256 = (
    "63ee82b2421cbb9b4c5346c72dbdb15e26f0ef8e0d2938357fb75228ef8c9a8b"
)
...
def _new_execution_checkpoint(
    randomness: cap.CAPRandomness, point_values: Sequence[int]
) -> dict[str, object]:
    return {
...
def _load_execution_checkpoint(
    path: Path, randomness: cap.CAPRandomness, point_values: Sequence[int]
) -> tuple[dict[str, object], bool]:
    if not path.exists():
        return _new_execution_checkpoint(randomness, point_values), False
    with path.open("rb") as stream:
        value = pickle.load(stream)
    if not isinstance(value, dict):
        raise ValueError("production tree execution checkpoint type mismatch")
    expected = _new_execution_checkpoint(randomness, point_values)
    for key in (
        "format",
        "relation_id",
        "profile_fingerprint",
...
def build_tree2_material(
    ...
    *,
    ...
    """Build or resume the real tree-2 XOF outputs and polynomial."""
    ...
    state, resumed = _load_execution_checkpoint(
        checkpoint_path, randomness, point_values
    )
    ...
    while len(nodes) < LEAVES:
        ...
        if progress is not None:
            progress(f"tree 2 execution checkpoint: {len(nodes)}/{LEAVES} seeds")
    ...
    while len(leaf_outputs) < LEAVES:
        ...
        _atomic_pickle(checkpoint_path, state)
        if progress is not None:
            progress(f"tree 2 execution checkpoint: {end}/{LEAVES} leaves")
    ...
    if tree_component_sha256 != FROZEN_TREE_COMPONENT_SHA256:
        raise AssertionError("tree-2 component disagrees with frozen v2.8 execution")
...
def build_production_tree2(
    ...
    *,
    ...
    with assignment.AssignmentArchiveReader(
        ...
        randomness, material, component_sha, resumed_cache = build_tree2_material(
            execution_cache_path,
            point_values,
            workers=workers,
            progress=progress,
        ...
        with assignment.AssignmentArchiveReader(
            ...
            if verified.verification_failures:
                raise AssertionError(
                    "production tree-2 replay failed first at "
            ...
            if (
                ...
                raise AssertionError("production tree-2 replay topology mismatch")
    ...
    if not all(probe.rejected for probe in standard_probes):
        raise AssertionError("a production tree-2 producer probe was accepted")
...
def build_manifest(
    ...
    return {
        ...
        "claim_boundary": {
            "production_index2_2048_degree12_producer_native_closed": native_closed,
            "production_index2_point_wire_identity_closed": point_identity,
            "production_index2_output_values_match_tail": all_outputs_match,
