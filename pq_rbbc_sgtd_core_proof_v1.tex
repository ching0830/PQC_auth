...
\author{Research construction draft, version 2.6}
\date{24 August 2026}
...
\midrule
Portable v2.6 relation & 685,571 & includes 1,152 native-overlap hardening rows\\
\bottomrule
\end{tabular}
\end{center}
...
above and records it in the version-2.6 manifest.  Fork signature and native
CAP estimates
...
Version 2.6 preserves and audits the incremental relation in
...
\texttt{pq\_rbbc\_br1cs\_manifest\_v2\_5.json}.
...
and a bounded-memory one-tree production-shape stream with 26,126,283 rows,
...
assignment satisfies every row and rejects five stale-witness probes (all CAP
fixtures non-secure and zero external assertions).\\
\addlinespace
...
exact inter-call wire identities across all trees; the $2^{12}$-leaf shard;
execution of a full
...
\begin{enumerate}[label=INV-\arabic*,leftmargin=*]
  ...
  relations, and version-2.6 assignment-backed 2,048-leaf streamed shard define
  the implemented
  $H_{\RBBC}$ wires unambiguously.  The CAP fixtures are explicitly non-secure
  and cannot satisfy the production import contract; the 4,096-leaf shard, all
  18 trees, and exact parent-circuit wire identities are still required.
...
well.  The full 2,048-bit witness and every degree-12 symbolic mask slice have
now been executed in one real 2,048-leaf production tree shape, with a frozen
bounded-memory row-stream digest, a complete fixed-width assignment, whole-row
verification, five stale-witness rejections, and zero external assertions.  The 240-row
...
\begin{quote}
\emph{Add the 4,096-leaf/degree-13 shard with the same complete-assignment and
whole-shard verification discipline.  Then compose all eighteen trees and their
