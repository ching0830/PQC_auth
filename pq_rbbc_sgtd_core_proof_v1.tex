...
\author{Research construction draft, version 1.9}
\date{23 August 2026}
...
\paragraph{Executable refinement in version 1.9.}
...
Version 1.9 preserves this relation and adds the native component probe below;
it does not weaken the remaining external CAP-hash boundary.
...
v1.9 archive are already part of a genuine native $\pi_1$ relation and are not
...
above and records it in the version-1.9 manifest.  Native level-III estimates
...
Version 1.9 preserves and audits the incremental relation in
...
\path{pq_rbbc_reference_manifest_v1_9.json}.
...
Version 1.9 preserves the proof-backend exchange layer in
...
excluding proof; a 12,012-byte online-ticket target; and a source-pinned
$\mathbb F_{2^{193}}$ Anemoi component with 336 nonlinear plus eight output
binding rows and a machine-checked parameter-gap audit.\\
\addlinespace
Not yet instantiated & A post-quantum zero-knowledge and
simulation-extractable implementation of $\Piiss$; a native
$\mathbb F_{2^{193}}$ Blind-UOV-III CAP-hash circuit import, a bit-exact
reproduction of the reported 240-constraint permutation, and validation of
...
\begin{enumerate}[label=INV-\arabic*,leftmargin=*]
  ...
  \item The version-1.9 Anemoi component is insufficient to satisfy the native
  import contract: it contains none of the seven complete CAP/GGM/transcript
  components and must never be relabelled as $H_{\BUOV}$.
  \item Production closure requires an authoritative parameter and constraint
  specification that independently resolves the reported 240 constraints
  against the public 336- and 360-row derivations, plus independent Sage
  vectors for the selected instance.
\end{enumerate}
...
lowering, native-import contract, and first native field/permutation component
are now complete.  The immediate next step is blocked on one authoritative
reproducibility input, followed by two coupled proof obligations:

\begin{quote}
\emph{Obtain from a published release or the Blind-UOV authors the bit-exact
level-III Anemoi parameter set and constraint specification that explains the
reported 240 rows.  Reproduce the count and outputs independently in Sage; do
not tune the local implementation merely to hit the number.  Then implement
all seven CAP/GGM/transcript components for Protocols 8--10, pin every source
and parameter file, and compile the complete CAP.Commit-plus-hash subrelation
over $\mathbb F_{2^{193}}$.  Feed it the exact digest, mask, and hash-image wires
already named by the import contract; the combined archive must contain zero
external assertions and pass all native tamper tests.  Validate the
straightline-extraction, unique-mask, commit-before-hash, and domain-separation
conditions used in Theorem~\ref{thm:j-qrom}.  Then select a concrete backend,
