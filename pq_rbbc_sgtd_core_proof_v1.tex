...
\author{Research construction draft, version 1.1}
\date{23 August 2026}
...
The main result is conditional and modular: assuming quantum-secure commitment,
simulation-extractable zero knowledge, Blind-UOV one-more unforgeability and
...
claim ordinary IND-CCA security for the bare trace encryption.  Version 1.1
instantiates the abstract blind-request relation with the published Blind-UOV
masking normal form and proves its ticket-message binding.  The exact CAP/SNARK
circuit, parameters, and implementation proof remain separate obligations.
\end{abstract}
...
only a ticket payload and a blind signature.
...
request equations are fixed in Section~\ref{sec:buov-normal}; converting the
entire ticket relation into the selected post-quantum CAP/SNARK circuit and
performing concrete parameter accounting remain implementation tasks.
...
\subsection{Hashing and commitment}
...
\[
  c_M\leftarrow \Com.\mathsf{Commit}(\pp_{\Com},M;\omega)
\]
is computationally binding and hiding against $\QPT$ adversaries.  Statistical
hiding is preferable because issuance privacy is then less dependent on an
algebraic assumption.

\subsection{Blind-UOV request normal form}\label{sec:buov-normal}
...
the exact hidden message represented by the request.  The Blind-UOV authors'
published construction admits the following paper-compatible normal form.  Let
$r\leftarrow\mathbb F_q^o$ be a fresh mask and let $\rho_0$ be commitment
randomness.  The user computes~\cite{blind-uov,blind-uov-slides}
\begin{align}
 c_0&\leftarrow \mathsf{CAP.Commit}(r;\rho_0),
   \tag{B1}\label{eq:b1}\\
 y&=r+H_{\BUOV}(m,c_0)\in\mathbb F_q^o.
   \tag{B2}\label{eq:b2}
\end{align}
The request object used by this core is $\beta=(c_0,y)$ and its blinding
witness is $b=(r,\rho_0)$.  The issuer applies the underlying UOV/hash-and-sign
trapdoor to the masked target $y$ only after accepting a proof that the request
is well built.  The user then runs the Blind-UOV finalization/CAP procedure to
obtain the publicly verifiable final signature $\sigma$ on $m$.

The exact byte-level CAP transcript is revision-dependent and is intentionally
left behind the algorithms
$\BUOV.\mathsf{Respond}$, $\BUOV.\mathsf{Finalize}$, and
$\BUOV.\mathsf{Vfy}$.  Equations~\eqref{eq:b1}--\eqref{eq:b2} are the only
Blind-UOV internals needed by the ticket-binding reduction.  The authors'
slides write $\mathsf{CP.Commit}$; this document writes
$\mathsf{CAP.Commit}$ for that commitment subroutine inside the CAP system.

\begin{definition}[Concrete Blind-UOV request relation]\label{def:rrbs}
For $\beta=(c_0,y)$ and $b=(r,\rho_0)$, define
\[
 R^{\BUOV}_{\mathsf{req}}(\beta;m,b)=1
 \quad\Longleftrightarrow\quad
 c_0=\mathsf{CAP.Commit}(r;\rho_0)\ \wedge\
 y=r+H_{\BUOV}(m,c_0).
\]
A completed accepted protocol must return a Blind-UOV final signature that
verifies on the same $m$.
\end{definition}

We require correctness, blindness against an honest-protocol curious signer,
and one-more unforgeability against malicious users in the QROM/security model
of the selected Blind-UOV revision.  Request binding is no longer an independent
assumption: Lemma~\ref{lem:buov-unique} derives it from CAP binding and collision
resistance of $H_{\BUOV}$.
...
a signature on $M_1$.  The $R^{\BUOV}_{\mathsf{req}}$ conjunct is the formal
...
The proof may be tens or hundreds of kilobytes because it is transmitted only
during infrequent issuance.  This design choice is intentional.  Knowledge
...
\begin{align*}
  ...
  C&\leftarrow \TEnc.\mathsf{Enc}(\mathsf{tpk},\mu;\rho),\\
...
\[
 R_{\mathsf{enc}}(\mathsf{tpk},C;\mu,\rho)=1
 \quad\Longleftrightarrow\quad
 C=\TEnc.\mathsf{Enc}(\mathsf{tpk},\mu;\rho).
\]
...
\begin{enumerate}[label=(T\arabic*),leftmargin=*]
  ...
  $R_{\mathsf{enc}}(\mathsf{tpk},C;\mu,\rho)=1$ and
  $R_{\mathsf{enc}}(\mathsf{tpk},C;\mu',\rho')=1$.
...
\[
 ...
 \qquad C=\TEnc.\mathsf{Enc}(\mathsf{tpk},\rid\|\sn;\rho_{\mathrm{tr}}).
\]
The serial number $\sn\leftarrow\{0,1\}^{\ell_{\sn}}$ is fresh.  It appears both
...
\begin{align*}
 x&=(\pp,\mathsf{ci},\mathsf{sid},\rid,c_M,\beta),\\
 w&=(M,\omega,r,\rho_0,k_{\mathrm{hold}},\rho_{\mathrm{tr}}).
\end{align*}
The relation $R_{\mathsf{issue}}(x;w)=1$ iff all of the following hold:
\begin{align}
 \mathsf{TicketShape}(M,\mathsf{ci})&=1, \tag{I1}\label{eq:i1}\\
 c_M&=\Com.\mathsf{Commit}(\pp_{\Com},M;\omega), \tag{I2}\label{eq:i2}\\
 m&=H_{\mathrm{ticket}}(\mathsf{Encode}(M)), \tag{I3}\label{eq:i3}\\
 R^{\BUOV}_{\mathsf{req}}(\beta;m,(r,\rho_0))&=1,
   \tag{I4}\label{eq:i4}\\
 M.h&=H_{\mathrm{hold}}(k_{\mathrm{hold}}), \tag{I5}\label{eq:i5}\\
 R_{\mathsf{enc}}(\mathsf{tpk},M.C;\rid\|M.\sn,\rho_{\mathrm{tr}})&=1.
   \tag{I6}\label{eq:i6}
\end{align}
The real identity and session identifier are public to the issuer during
issuance but are not part of $M$.  Equation~\eqref{eq:i6} binds the hidden
...
\begin{enumerate}[leftmargin=*]
  \item generate $\Piiss$ parameters and the commitment/hash parameters;
  \item generate issuer keys
  $(\mathsf{bsk},\mathsf{bvk})\leftarrow\BUOV.\mathsf{KeyGen}(1^\lambda)$;
  \item generate independent opening keys
  $(\mathsf{tpk},\mathsf{tsk}_1,\ldots,\mathsf{tsk}_n)
  \leftarrow\TEnc.\mathsf{KeyGen}(1^\lambda,n,\tau)$;
  \item publish the common epoch information $\mathsf{ci}$ authenticated by a
  federation configuration signature.
\end{enumerate}
...
\begin{enumerate}[leftmargin=*]
  \item The holder samples $\sn,k_{\mathrm{hold}},\rho_{\mathrm{tr}}$, computes
  $C=\TEnc.\mathsf{Enc}(\mathsf{tpk},\rid\|\sn;\rho_{\mathrm{tr}})$,
  constructs $M$, and
  computes $m=H_{\mathrm{ticket}}(\mathsf{Encode}(M))$.
  \item It samples $\omega,r,\rho_0$, computes
  \[
    c_M=\Com.\mathsf{Commit}(M;\omega),\qquad
    c_0=\mathsf{CAP.Commit}(r;\rho_0),\qquad
    y=r+H_{\BUOV}(m,c_0),
  \]
  sets $\beta=(c_0,y)$, and produces
  \[
     \pi_{\mathsf{issue}}
  ...
  $s\leftarrow\BUOV.\mathsf{Respond}(\mathsf{bsk},y)$.
  \item The holder computes
  \[
    \sigma\leftarrow
    \BUOV.\mathsf{Finalize}(m,c_0,y,s;r,\rho_0)
  \]
  and accepts only if
...
The spendable ticket is $T=(M,\sigma)$.  Neither $c_M$, $\beta$,
...
$m=H_{\mathrm{ticket}}(\mathsf{Encode}(M))$, checks canonical encoding and the
public common-information policy, then returns
$\BUOV.\mathsf{Vfy}(\mathsf{bvk},m,\sigma)$.
...
\begin{enumerate}[label=(\alph*),leftmargin=*]
  ...
  Equations~\eqref{eq:i1}--\eqref{eq:i6}.
\end{enumerate}
...
$\Piiss$ accepts.  Blind-signature correctness gives a verifying $\sigma$ on
$H_{\mathrm{ticket}}(\mathsf{Encode}(M))$.  Equation~\eqref{eq:i6} gives a
...
\begin{lemma}[Blind-UOV request uniqueness]\label{lem:buov-unique}
Let $\beta=(c_0,y)$.  Suppose
$R^{\BUOV}_{\mathsf{req}}(\beta;m,(r,\rho_0))=1$ and
$R^{\BUOV}_{\mathsf{req}}(\beta;m',(r',\rho'_0))=1$ for $m\neq m'$.
Then one can derive either two different openings of the same CAP commitment or
a collision in $H_{\BUOV}$.
\end{lemma}

\begin{proof}
Both witnesses open $c_0$.  If $r\neq r'$, they violate CAP binding.  Otherwise
$r=r'$, and the two equations for the same $y$ imply
\[
 H_{\BUOV}(m,c_0)=y-r=H_{\BUOV}(m',c_0),
\]
which is a collision because $m\neq m'$.  Thus the request identifies at most
one hidden message, except with the stated bad-event probability.
\end{proof}

\begin{lemma}[Request/proof binding]\label{lem:reqbind}
Suppose an issuance proof for $(c_M,\beta)$ is accepting and extractable.  Except
with ticket-commitment binding, CAP binding, or hash-collision probability, the
message signed through $\beta$ is exactly
$H_{\mathrm{ticket}}(\mathsf{Encode}(M))$ for the unique $M$ extracted from
$c_M$.
\end{lemma}

\begin{proof}
Extraction yields $(M,\omega,r,\rho_0,\ldots)$ satisfying
Equations~\eqref{eq:i2}--\eqref{eq:i4}.  Binding makes $M$ the unique feasible
opening of $c_M$.  Equation~\eqref{eq:i3} fixes $m$ to the canonical ticket
digest.  Lemma~\ref{lem:buov-unique} makes the message represented by $\beta$
equal to this $m$.  A different payload with the same $m$ yields a collision in
$H_{\mathrm{ticket}}$.
\end{proof}

\begin{theorem}[Certified-language soundness]\label{thm:certsound}
For any $\QPT$ adversary $\mathcal A$, there exist $\QPT$ reductions
$\mathcal B_1,\ldots,\mathcal B_6$ such that
\begin{align*}
 \Adv^{\mathsf{CertSound}}_{\Core,\mathcal A}
 \leq{}&
 \Adv^{\mathsf{SE\text{-}AoK}}_{\Piiss,\mathcal B_1}
 +\Adv^{\mathsf{1mUF}}_{\BUOV,\mathcal B_2}
 +\Adv^{\mathsf{bind}}_{\Com,\mathcal B_3}\\
 &+\Adv^{\mathsf{bind}}_{\mathsf{CAP},\mathcal B_4}
 +\Adv^{\mathsf{coll}}_{H_{\BUOV},\mathcal B_5}
 +\Adv^{\mathsf{coll}}_{H_{\mathrm{ticket}},\mathcal B_6}.
\end{align*}
...
Lemma~\ref{lem:reqbind} associates the signed digest with the unique committed
payload, except under the remaining binding and collision bad events.  The extracted witness
...
bounds the change.  Replace each commitment to the real payload with a
commitment to a fixed equal-length dummy string; hiding bounds the change.
The remaining request/response views are blind-signature views on two hidden
messages.  Blindness permits swapping those messages without the issuer
...
extracted accepted issuance witness.  Equation~\eqref{eq:i6} states that its
...
\begin{align*}
 ...
 +\Adv^{\mathsf{bind}}_{\Com,\mathcal B_4}
 +\Adv^{\mathsf{bind}}_{\mathsf{CAP},\mathcal B_5}\\
 &+\Adv^{\mathsf{coll}}_{H_{\BUOV},\mathcal B_6}
 +\Adv^{\mathsf{coll}}_{H_{\mathrm{ticket}},\mathcal B_7}.
\end{align*}
\end{theorem}
...
record
\[
  (d_M,M,\rid,\rho_{\mathrm{tr}},k_{\mathrm{hold}},r,\rho_0,\ldots)
\]
...
sides of the genuine Blind-UOV protocol and obtains an exactly distributed
final signature; it does not need the unknown trace-encryption randomness.
...
distinct verifying ticket digests has probability at most the one-more
unforgeability advantage of $\BUOV$ plus the CAP-binding and hash-collision
terms.
\end{theorem}
...
sessions directly form a one-more forgery unless two payloads share a digest or
Lemma~\ref{lem:buov-unique} fails.
\end{proof}

\section{Candidate concrete instantiations}

\subsection{Augmented-CAP Blind-UOV compiler}\label{sec:augcap}
...
second independent proof.  Let
\[
  R_{\mathsf{wb}}(c_0,y;m,r,\rho_0)=1
\]
denote Equations~\eqref{eq:b1}--\eqref{eq:b2}.  The original well-built-request
language is replaced by
\begin{align*}
 R_{\mathsf{aug}}(&\pp,\mathsf{ci},\mathsf{sid},\rid,c_M,c_0,y;\\
                  &M,\omega,r,\rho_0,k_{\mathrm{hold}},\rho_{\mathrm{tr}})=1
 \quad\Longleftrightarrow\quad
 R_{\mathsf{issue}}(x;w)=1,
\end{align*}
where $\beta=(c_0,y)$ and Equation~\eqref{eq:i4} expands to
$R_{\mathsf{wb}}=1$.  The issuer executes the ordinary Blind-UOV response only
after verifying a post-quantum SE-NIZK argument for $R_{\mathsf{aug}}$.

\paragraph{Compiler interface.}
The integration requires only one hook in a Blind-UOV implementation:
\[
 \mathsf{VerifyWellBuilt}(c_0,y,\pi_{\mathsf{wb}})
 \quad\longrightarrow\quad
 \Piiss.\mathsf{Vfy}(x,\pi_{\mathsf{issue}}).
\]
The response and finalization algorithms remain unmodified.  In particular,
the spendable signature is the genuine Blind-UOV final object.  It need not be,
and is not assumed to be, distributed as an ordinary direct UOV signature.
...
\begin{align*}
 \mathsf{pub}_{\mathsf{aug}}
   &= (\mathsf{ci},\mathsf{sid},\rid,\mathsf{tpk},\mathsf{bvk},c_M,c_0,y),\\
 \mathsf{wit}_{\mathsf{aug}}
   &= (\sn,k_{\mathrm{hold}},\rho_{\mathrm{tr}},r,\rho_0,\omega).
\end{align*}
...
\[
 ...
 \mathcal C_{\mathsf{trace}}\wedge
 \mathcal C_{\mathsf{ticketCom}},
\]
where
\begin{align*}
 \mathcal C_{\mathsf{shape}} &: \mathsf{TicketShape}(M,\mathsf{ci})=1,\\
 \mathcal C_{\mathsf{ticketHash}} &: m=H_{\mathrm{ticket}}(\mathsf{Encode}(M)),\\
 \mathcal C_{\mathsf{BUOVmask}} &: c_0=\mathsf{CAP.Commit}(r;\rho_0)
   \ \wedge\ y=r+H_{\BUOV}(m,c_0),\\
 \mathcal C_{\mathsf{holder}} &: h=H_{\mathrm{hold}}(k_{\mathrm{hold}}),\\
 \mathcal C_{\mathsf{trace}} &: C=\TEnc.\mathsf{Enc}
   (\mathsf{tpk},\rid\|\sn;\rho_{\mathrm{tr}}),\\
 \mathcal C_{\mathsf{ticketCom}} &: c_M=\Com.\mathsf{Commit}(M;\omega).
\end{align*}
...
\begin{theorem}[Security inheritance of augmented CAP]\label{thm:augcap}
...
honest-protocol signer blindness, up to the SE-NIZK, ticket-commitment hiding,
CAP-binding, and hash-collision terms already shown in
...
augmented proof itself and forwards only accepted $(c_0,y)$ requests to the
original signer.  If that interface requires the native well-built proof, the
reduction extracts $(m,r,\rho_0)$ from the augmented proof and generates the
native proof; extraction failure is charged to the SE-AoK term.  The augmented
relation is a restriction of the original well-built language, so every
forwarded request is valid there.  The final signature format and verifier are
unchanged.

For blindness, replace the augmented proof by a simulated proof and $c_M$ by a
commitment to a fixed equal-length dummy payload.  The remaining transcript is
an ordinary Blind-UOV transcript, to which its blindness game applies.  The
...
\paragraph{Concretization achieved in version 1.1.}
The request and witness are now fixed:
\begin{align*}
 \beta&=(c_0,y),& b&=(r,\rho_0),\\
 c_0&=\mathsf{CAP.Commit}(r;\rho_0),&
 y&=r+H_{\BUOV}(H_{\mathrm{ticket}}(\mathsf{Encode}(M)),c_0).
\end{align*}
Consequently, the earlier abstract ``request-binding'' assumption has been
eliminated.  It is replaced by the standard, auditable assumptions that the CAP
commitment is binding and the two domain-separated hashes are collision
resistant.

The authors' S\&P 2026 slides report a short-signature trade-off of roughly
5--8~KB with a few megabytes of blind-issuance communication.  The ePrint
revision reports a wider 3.8--11~KB signature range.  This is aligned with the
present objective because issuance is unconstrained while the online ticket is
not.  Exact numbers must be recomputed after adding $R_{\mathsf{aug}}$; the
augmented proof can be substantially larger than the paper's base
well-built-request proof~\cite{blind-uov,blind-uov-slides}.

\paragraph{Reference backend decision.}
The conservative first implementation will use the same MPC-in-the-Head/CAP
family as Blind-UOV and compile $\mathcal C_{\mathsf{aug}}$ as a generic
post-quantum circuit, accepting multi-megabyte issuance.  This keeps the final
Blind-UOV verifier unchanged and avoids introducing a second proof paradigm.
...
A candidate compact trace ciphertext is a threshold Niederreiter KEM combined
with authenticated symmetric encryption:
\begin{align*}
 c_{\mathrm{syn}} &= H_{\mathrm{pub}}e^\top,
   \qquad \mathrm{wt}(e)=t,\\
 K &= \mathsf{KDF}(e,c_{\mathrm{syn}},\mathsf{ci}),\\
 c_{\mathrm{dem}} &=
   \mathsf{AEAD.Enc}_{K}(\rid\|\sn;\mathsf{ad}=\mathsf{ci}),\\
 C&=(c_{\mathrm{syn}},c_{\mathrm{dem}}).
\end{align*}
The issuance relation proves knowledge of $(e,\rid,\sn)$ satisfying the
syndrome, weight, KDF, AEAD, and external-serial equations.  The distributed
decoder is invoked only after signature gating.

This candidate deliberately omits the public NIZK validity proof attached to
each ciphertext in recent threshold-Niederreiter CCA constructions.  Therefore
its security target is Theorem~\ref{thm:gcca}, not the base construction's
ordinary threshold IND-CCA definition.  A concrete proof must still establish:
\begin{itemize}[leftmargin=*]
  \item threshold IND-CPA privacy for the exact distributed key generation and
  corruption model;
  \item unique opening of the KEM--AEAD composition;
  \item robust partial-decryption shares;
  \item constant-time failure behavior and the absence of a raw decoder API.
\end{itemize}
...
\midrule
Proved at the abstract level & Correctness; proof/request/message binding;
...
\addlinespace
Assumed component property & Post-quantum SE-NIZK AoK, hiding/binding ticket
commitment, CAP binding, Blind-UOV blindness and one-more unforgeability,
threshold IND-CPA encryption with unique opening and robust shares, and
authorization unforgeability.\\
\addlinespace
Partially instantiated & The Blind-UOV request object, mask witness, augmented
relation, request-uniqueness lemma, and security-inheritance theorem are fixed.
The byte-level CAP/SNARK circuit is not yet selected.\\
\addlinespace
Not yet instantiated & Concrete SE-NIZK and CRS model, exact threshold
Niederreiter KEM--AEAD circuit, parameter sets, byte counts, and implementation
benchmarks.\\
\addlinespace
...
The next step is not satellite integration.  It is a concrete instantiation
lemma:

\begin{quote}
\emph{Freeze one Blind-UOV revision and parameter set, then instantiate
$\mathcal C_{\mathsf{trace}}$ with an exact threshold-Niederreiter KEM--AEAD
relation.  Arithmetize all six blocks in the selected MPC-in-the-Head/CAP
backend and count multiplication gates, hash invocations, proof bytes, prover
memory, and issuer verification cost.}
\end{quote}

This step will decide whether one monolithic proof is viable or whether
$R_{\mathsf{aug}}$ must be split into compositionally linked proofs.  It is the
last missing concrete component before deriving end-to-end byte counts; the
satellite AKE remains out of scope until this core closes.
