...
\author{Research construction draft, version 1.6}
\date{23 August 2026}
...
holder obtains a pair of blind signatures only after proving that the hidden ticket, the
...
count.  Version 1.6 corrects the signer-view ABI to the exact Blind-UOV
protocol: the signer sees only $y$, while the CAP commitment, message, mask,
and commitment randomness remain hidden until finalization.  It also identifies
a cross-message claw obligation absent from ordinary blind-signature security.
The aggressive profile therefore uses two independent, domain-separated
Blind-UOV-Is lanes.  Its online ticket is 7,912 bytes; its security theorem is
conditional on a QROM claw-resistance reduction and native circuit integration.
\end{abstract}
...
only a ticket payload and two compact blind signatures.
...
A single Blind-UOV-Is target is 256 bits.  Our authorization-binding property
is a freely chosen cross-message collision/claw problem, not a fixed-target
preimage problem.  Generic quantum collision algorithms therefore give only
about $2^{256/3}=2^{85}$ work~\cite{bht,compressed-oracle}.  The aggressive
post-quantum profile runs two independently randomized and domain-separated
lanes $i\in\{0,1\}$.  Define
$\mu_i=\mathsf{Encode}(\texttt{PQ-RBBC/BUOV-LANE},i,m)$ and compute:
\begin{align}
 c_{r,i}&\leftarrow \mathsf{CAP.Commit}_i(r_i;\rho_i),
   \tag{B3}\label{eq:b3}\\
 y_i&=J^i_m(r_i,\rho_i)
 =r_i+H_{\BUOV}(\mu_i,c_{r,i}).
   \tag{B4}\label{eq:b4}
\end{align}
The public request is exactly $\beta=(y_0,y_1)$; its hidden witness is
$b=((r_0,\rho_0),(r_1,\rho_1))$.  Masks and commitment randomness MUST be
independent across lanes.  The issuer returns two ordinary Blind-UOV responses,
and finalization produces ordinary signatures on $\mu_0$ and $\mu_1$.  They
bind one logical ticket digest $m$ but are not byte-identical messages.

\begin{definition}[Dual Blind-UOV request relation]\label{def:rrbs}
For $\beta=(y_0,y_1)$ and
$b=((r_0,\rho_0),(r_1,\rho_1))$, define
\[
 R^{\BUOV,2}_{\mathsf{req}}(\beta;m,b)=1
 \Longleftrightarrow
 \bigwedge_{i=0}^{1}
 \left[c_{r,i}=\mathsf{CAP.Commit}_i(r_i;\rho_i)\ \wedge\
 y_i=r_i+H_{\BUOV}(\mu_i,c_{r,i})\right].
\]
The $c_{r,i}$ are derived hidden witnesses.  A completed session returns
$\sigma_0,\sigma_1$, where $\sigma_i$ verifies on the canonical $\mu_i$
derived from the same $m$.
\end{definition}

\begin{assumption}[Dual-lane cross-message request claw resistance]
\label{ass:dual-claw}
No $\QPT$ adversary can output $m\neq m'$ and valid independent openings for
both sides such that
\[
 (J^0_m(r_0,\rho_0),J^1_m(r_1,\rho_1))
 =
 (J^0_{m'}(r'_0,\rho'_0),J^1_{m'}(r'_1,\rho'_1))
\]
except with negligible probability.  For ideal independent 256-bit lane
outputs, the generic quantum collision scale is about
$2^{512/3}=2^{170}$.  This estimate motivates the construction but is not a
completed QROM reduction for the structured $J^i$ maps.
\end{assumption}

We additionally require correctness, parallel two-instance blindness, and
one-more unforgeability of Blind-UOV.  Assumption~\ref{ass:dual-claw} is a new
composition obligation; it is not implied merely by CAP binding and ordinary
collision resistance of $H_{\BUOV}$.
...
a signature on $M_1$.  The $R^{\BUOV,2}_{\mathsf{req}}$ conjunct is the formal
...
w&=(M,(r_0,\rho_0),(r_1,\rho_1),k_{\mathrm{hold}},e).
\end{align*}
...
\begin{align}
 ...
 R^{\BUOV,2}_{\mathsf{req}}
   (\beta;m,((r_0,\rho_0),(r_1,\rho_1)))&=1,
   \tag{I3}\label{eq:i3}\\
...
\begin{enumerate}[leftmargin=*]
  ...
  \item For $i\in\{0,1\}$ it independently samples $r_i,\rho_i$, computes
  \begin{align*}
    \mu_i&=\mathsf{Encode}(\texttt{PQ-RBBC/BUOV-LANE},i,m),\\
    c_{r,i}&=\mathsf{CAP.Commit}_i(r_i;\rho_i),&
    y_i&=r_i+H_{\BUOV}(\mu_i,c_{r,i}).
  \end{align*}
  sets the public request $\beta=(y_0,y_1)$, and produces
  \[
     \pi_{\mathsf{issue}}
  ...
  \]
  Only then does it execute, for both lanes,
  $z_i\leftarrow\BUOV.\mathsf{Respond}(\mathsf{bsk},y_i)$.
  \item For both lanes the holder computes
  \[
    \sigma_i\leftarrow
    \BUOV.\mathsf{Finalize}(\mu_i,c_{r,i},y_i,z_i;r_i,\rho_i)
  \]
  and accepts only if both
  $\BUOV.\mathsf{Vfy}(\mathsf{bvk},\mu_i,\sigma_i)=1$.
\end{enumerate}
...
relation.  Thus issuance sends one large relation proof, not two independent
proofs.

The spendable ticket is $T=(M,\sigma_0,\sigma_1)$.  Neither $\beta$,
...
$m=H_{\mathrm{ticket}}(\mathsf{Encode}(M))$ and both canonical $\mu_i$
encodings, checks canonical payload encoding and the public
common-information policy, then returns one iff
\[
 \BUOV.\mathsf{Vfy}(\mathsf{bvk},\mu_0,\sigma_0)=
 \BUOV.\mathsf{Vfy}(\mathsf{bvk},\mu_1,\sigma_1)=1.
\]
...
$T=(M,\sigma_0,\sigma_1)$ for which $\Core.\mathsf{VerifyTicket}(T)=1$ and either:
...
$T=(M,\sigma_0,\sigma_1)$ verifies.  Any authorized set of at least $\tau$ honest opening
...
$\Piiss$ accepts.  Blind-signature correctness gives two verifying signatures
on the two lane encodings of $H_{\mathrm{ticket}}(\mathsf{Encode}(M))$.
...
\begin{lemma}[Conditional dual-request uniqueness]\label{lem:buov-unique}
Let $\beta=(y_0,y_1)$.  If the dual request relation accepts valid openings for
both $m$ and $m'\neq m$, then those values constitute a winning output in the
game of Assumption~\ref{ass:dual-claw}.
\end{lemma}

\begin{proof}
Expanding the two accepted relations gives two independently opened pairs of
$J$ values equal to the same public $(y_0,y_1)$ for distinct messages.  This is
exactly the dual cross-message claw game.  Notice that CAP binding alone cannot
rule it out because the two sides may use different commitments.
\end{proof}
...
with dual-request claw probability, the message signed through
...
\begin{proof}
Extraction yields $(M,(r_0,\rho_0),(r_1,\rho_1),\ldots)$ satisfying
...
\begin{align*}
 ...
 +2\Adv^{\mathsf{1mUF}}_{\BUOV,\mathcal B_2}
 +\Adv^{\mathsf{dualClaw}}_{J,\mathcal B_3}
...
\begin{proof}
Consider a verifying output $(M,\sigma_0,\sigma_1)$.  If its canonical digest was not the
...
If $\Core.\mathsf{VerifyTicket}(M,\sigma_0,\sigma_1)=1$, then $M.C\in
...
bounds the change.  The remaining request/response views are two parallel
blind-signature views per hidden message.  A two-hybrid argument applies
Blind-UOV blindness once per lane and permits swapping those messages without the issuer
detecting the swap.  Threshold-encryption privacy prevents the issuer, which
...
\begin{align*}
 ...
 +\Adv^{\mathsf{dualClaw}}_{J,\mathcal B_4}\\
 &
...
record
\[
  (d_M,M,\rid,e,k_{\mathrm{hold}},(r_0,\rho_0),(r_1,\rho_1),\ldots)
\]
...
key and knows the complete challenge message $m^*$, it locally executes both
sides of both genuine Blind-UOV lanes and obtains two exactly distributed final
signatures; it does not need the unknown trace-encryption randomness.
...
After $q$ completed accepted dual-lane blind-issuance sessions, producing more
than $q$ distinct verifying ticket digests has probability at most the
one-more-unforgeability advantage of either projected Blind-UOV lane, plus the
dual-claw and ticket-hash collision terms.
\end{theorem}

\begin{proof}
Map every completed issuance to the unique signed digest using
Lemma~\ref{lem:reqbind}.  Project every complete ticket onto lane 0.  More
distinct verifying digests than completed sessions then give more valid lane-0
message/signature pairs than lane-0 responses, unless two payloads share a
digest or Lemma~\ref{lem:buov-unique} fails.  Lane 1 remains mandatory for the
cross-message binding target; it is not needed for this counting projection.
\end{proof}
...
\[
  R_{\mathsf{wb}}^i(y_i;m,r_i,\rho_i)=1
\]
denote Equations~\eqref{eq:b3}--\eqref{eq:b4} for lane $i$.  The two native
well-built-request languages are replaced by one augmented language:
\begin{align*}
 R_{\mathsf{aug}}(&\pp,\ctx,\mathsf{sid},\rid,y_0,y_1;\\
                  &M,(r_0,\rho_0),(r_1,\rho_1),k_{\mathrm{hold}},e)=1
 \quad\Longleftrightarrow\quad
 R_{\mathsf{issue}}(x;w)=1,
\end{align*}
where $\beta=(y_0,y_1)$ and Equation~\eqref{eq:i3} expands to
$R_{\mathsf{wb}}^0\wedge R_{\mathsf{wb}}^1$.  The issuer executes the two
ordinary Blind-UOV responses only
after verifying a post-quantum SE-NIZK argument for $R_{\mathsf{aug}}$.

\paragraph{Compiler interface.}
The integration requires a two-lane wrapper around the native verification hook:
\[
 \mathsf{VerifyWellBuilt}(y_i,\pi_{1,i})_{i=0,1}
 \quad\longrightarrow\quad
 \Piiss.\mathsf{Vfy}(\pp,\ctx,\mathsf{sid},\rid,y_0,y_1,
                    \pi_{\mathsf{issue}}).
\]
Each per-lane response and finalization algorithm remains unmodified.  The
spendable objects are two genuine Blind-UOV final signatures.
...
\begin{align*}
 \mathsf{pub}_{\mathsf{aug}}
   &= (\ctx,\mathsf{sid},\rid,\mathsf{tpk},\mathsf{bvk},y_0,y_1),\\
 \mathsf{wit}_{\mathsf{aug}}
   &= (\sn,k_{\mathrm{hold}},e,(r_0,\rho_0),(r_1,\rho_1)).
\end{align*}
...
\begin{align*}
 ...
 \mathcal C_{\mathsf{BUOVmask}} &: \bigwedge_{i=0}^{1}
   \left[c_{r,i}=\mathsf{CAP.Commit}_i(r_i;\rho_i)\ \wedge\
   y_i=r_i+H_{\BUOV}(\mu_i,c_{r,i})\right],\\
...
$\pi_{\mathsf{issue}}$ preserves the projected Blind-UOV one-more property and
parallel honest-protocol signer blindness, up to the SE-NIZK, dual-claw, and
...
augmented proof itself and forwards each accepted $y_i$ to its corresponding
original signer instance.  If that interface requires the native well-built
proof, the reduction extracts $(m,r_i,\rho_i)$ and generates it.  Extraction
failure is charged to SE-AoK.  The augmented relation restricts both original
well-built languages; each final signature format and verifier is unchanged.

For blindness, replace the augmented proof by a simulated proof.  The remaining
transcripts are ordinary Blind-UOV transcripts; a two-hybrid argument applies
its blindness game once per lane.  The
...
\paragraph{Corrected concretization in version 1.6.}
The request and witness are now fixed:
\begin{align*}
 \beta&=(y_0,y_1),& b&=((r_0,\rho_0),(r_1,\rho_1)),\\
 \mu_i&=\mathsf{Encode}(\texttt{PQ-RBBC/BUOV-LANE},i,
 H_{\mathrm{ticket}}(\mathsf{Encode}(M))),\\
 c_{r,i}&=\mathsf{CAP.Commit}_i(r_i;\rho_i),&
 y_i&=r_i+H_{\BUOV}(\mu_i,c_{r,i}).
\end{align*}
The signer sees only the 64-byte pair $(y_0,y_1)$, not $c_{r,i}$ or $m$.
...
attacker chooses different commitments on both sides.  Version 1.6 restores the
paper ABI and states Assumption~\ref{ass:dual-claw} explicitly.  The removed
...
Version 1.3 fixes the 31 October 2025 revision and its
\emph{Blind-UOV-Is, NIST-I, Shorter, TCitH} row.  The paper gives
$\bar n_r=256$ bits, $\bar n_x=640$ bits, a 66.6~KB public key, and a
3,772-byte final signature per lane.  Its NIST-I/Shorter TCitH parameters are
$\tau=12$, ten $2^{11}$-leaf trees plus two $2^{10}$-leaf trees,
$T_{\mathrm{open}}=111$, an explicit proof-of-work of 9 bits, and total
proof-of-work 14.0 bits~\cite{blind-uov}.  These values minimize the stored and
online object; issuance is intentionally the expensive side of the trade-off.
...
Consequently each 3,772-byte final signature remains unchanged.  The dual
profile carries two such signatures, while the ticket/trace blocks are shared
inside one augmented issuance relation.

For costing, use the paper's illustrative Shockwave/Anemoi row.  For one native
lane Blind-UOV-Is/Shorter is reported as approximately 16 million R1CS
constraints, a 2.8~MB proof, 32~s proving, and 2.5~s verification.  The dual
baseline is therefore approximately 32 million constraints before adding the
shared ticket/trace relation.  The paper
...
cached public key buys a 288-byte trace ciphertext.  The dual Blind-UOV-Is
profile and exact 7,912-byte complete ticket are derived in
...
constraints added beyond two copies of the paper's approximately
16-million-constraint Blind-UOV-Is/Shorter $\pi_1$ baseline.
...
$\mathcal C_{\mathsf{BUOVmask}}$ & 0 & already in the paper baseline\\
...
\midrule
Added relation & 684,419 & exact for the specified gadgets\\
\bottomrule
\end{tabular}
\end{center}
...
Because the paper rounds each lane baseline to 16 million constraints, the combined
number is itself approximate:
\[
 N_{\pi_{\mathsf{issue}}}\approx2(16{,}000{,}000)+684{,}419
 =32{,}684{,}419,
\]
or 2.0428 times the one-lane paper anchor.  Applying the paper's square-root
proof-size scaling and linear prover scaling gives approximately 4.00~MB and
65.4~s.  A conservative linear scaling of its verifier figure gives 5.11~s.
These are extrapolations from Table~5 of the Blind-UOV paper, not local
benchmarks.  The paper does not provide enough information to derive peak
prover memory, so memory remains a measurement obligation.
...
the dual-lane well-built-request NIZK changes its baseline by about 2.14\%.
...
\]
Combining this with two published 3,772-byte Blind-UOV-Is/Shorter signatures
gives
\[
 |T|=|M|+|\sigma_0|+|\sigma_1|=7{,}912\text{ bytes}
\]
before transport framing.  The cached public material is approximately
1,044,992 bytes for the trace key plus the paper's rounded 66.6~KB Blind-UOV
verification key, reused by both lanes.  Relative to a 70~KB online credential,
the frozen target is about 8.9 times smaller.  The approximately 4.00~MB
issuance NIZK is offline and
is never forwarded to a satellite verifier.

\begin{remark}[Audit reproducibility]
The companion script \texttt{pq\_rbbc\_circuit\_audit.py} recomputes every
integer above and emits the checked
\texttt{pq\_rbbc\_circuit\_audit\_v1\_3.json}.  Exact gadget counts and
paper-derived estimates are kept in separate JSON fields so a rounded source
number cannot be mistaken for a benchmark.
\end{remark}

\subsection{Executable reference relation and rejection tests}
\label{sec:executable-relation}

Version 1.6 extends the incremental relation in
...
\midrule
Allocated wire identifiers & 2,976,784\\
Linear definitions & 2,281,323\\
Linear equality assertions & 2,958\\
Nonlinear constraints & 684,419\\
Public input bits & 3,968\\
External Blind-UOV assertions & 2\\
...
The regression suite accepts the honest witness and sends eight independent
...
changed blind request, and changed common context.  All eight are rejected.
The checked output is recorded in
\path{pq_rbbc_reference_manifest_v1_6.json}.
...
\texttt{TestBlindUOVAdapter} checks message/request binding with a deterministic
SHAKE construction, but is not a blind signature.  It marks two native
Blind-UOV $\pi_1$/CAP boundaries whose nonlinear cost remains in the dual
paper-anchored baseline.  The 512 public $y_0,y_1$ bits are serialized as
circuit inputs, while $m,r_i,\rho_i,c_{r,i}$ remain hidden.  Consequently this implementation establishes that the incremental
...
Version 1.6 uses the proof-backend exchange layer in
...
Nonlinear R1CS rows & 684,419\\
Materialized affine-definition rows & 2,281,323\\
Linear assertion rows & 2,958\\
Total R1CS rows & 2,968,700\\
Archive size & 49,183,966 bytes\\
\bottomrule
\end{tabular}
\end{center}
The 684,419 figure is therefore a \emph{nonlinear-only} cost under the affine
elimination model used in Section~\ref{sec:circuit-audit}.  The 2,968,700 figure
...
the archive compatible in principle with the $\mathbb F_{2^{129}}$ cost anchor,
without claiming that a specific prover has been integrated.

An independent reader reconstructs and evaluates all 2,968,700 rows.  It
...
wrong-weight witness produces exactly the same 49,183,842-byte body and the
...
end-to-end backend tests completed in approximately 77.9 seconds in the
reference environment.  The checked metadata is recorded in
\texttt{pq\_rbbc\_br1cs\_manifest\_v1\_6.json}.

\paragraph{Remaining external rows.}
The archive records two named external assertions for the native Blind-UOV
$\pi_1$/CAP request relations, but cannot evaluate them.  This is deliberate and
machine-visible: an archive with an unchecked external assertion is not a
closed issuance proof.  The next integration must replace that marker with the
actual Blind-UOV constraints; deleting or treating either as automatically true is
forbidden.
...
\midrule
Conditional abstract reductions & Correctness; dual proof/request/message binding;
...
\addlinespace
Assumed component property & Post-quantum SE-NIZK AoK; dual-lane cross-message
request claw resistance; parallel Blind-UOV blindness and projected one-more
...
\addlinespace
Concretely instantiated & Blind-UOV-Is/Shorter costing profile; executable
...
full-circuit positive and negative tests; a checksummed portable R1CS archive
with independent row evaluation; 684,419 incremental nonlinear rows; 2,968,700
materialized total rows; a 64-byte issuance request excluding proof; and a
7,912-byte online-ticket target.\\
\addlinespace
Not yet instantiated & A post-quantum zero-knowledge and
simulation-extractable implementation of $\Piiss$; a structured-QROM reduction
for Assumption~\ref{ass:dual-claw}; two native Blind-UOV circuit imports;
...
\begin{enumerate}[label=INV-\arabic*,leftmargin=*]
  ...
  \item The public issuance request contains exactly $(y_0,y_1)$; it contains
  neither $m$ nor either $c_{r,i}$.  The two lanes use independent masks and
  commitment randomness plus explicit lane-domain separation.
  ...
  production issuance; both Blind-UOV markers must be replaced, never ignored.
\end{enumerate}

\section{Next proof step}

The incremental relation, unique opening, fixed layout, circuit audit,
executable witness builder, rejection tests, and portable R1CS lowering are now
complete.  The next step has two coupled proof obligations:

\begin{quote}
\emph{First prove Assumption~\ref{ass:dual-claw} for the structured,
domain-separated $J^0,J^1$ maps in the QROM, or replace the combiner with one
that has such a theorem.  In parallel, replace both external markers with the
native Blind-UOV $\pi_1$/CAP constraints and prove that both message inputs are
exactly the 256-bit digest produced by
$\mathcal C_{\mathsf{ticketHash}}$.  Then select a concrete backend,
