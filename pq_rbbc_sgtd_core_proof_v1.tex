...
\author{Research construction draft, version 2.5}
\date{24 August 2026}
...
constraint generator.  Version 2.5 uses
...
run for this release.  Version 2.5 runs one 2,048-leaf tree shape, but no
18-tree native row stream, complete shard assignment, or parent inter-call
...
The proposition is intentionally about topology, not full shard witness
evaluation or production security.  The implementation does not archive a
complete 19,903,324-wire assignment, so it cannot yet supply a backend-checked
honest assignment or stale-witness mutation proof for the whole shard.  The
profile also contains only one tree, omits all cross-tree corrections, and is
explicitly marked non-secure.  Those omissions prevent any inference that the
full CAP relation, Blind-UOV fork, or $\pi_{\mathsf{issue}}$ is closed.
...
comparator, not a version-2.5 fork estimate.
...
\midrule
Portable v2.5 relation & 685,571 & includes 1,152 native-overlap hardening rows\\
\bottomrule
\end{tabular}
\end{center}
...
version-2.5 signature size.  Under the same provisional assumption, cached
...
figure is a superseded historical comparator; version 2.5 already counts more
...
above and records it in the version-2.5 manifest.  Fork signature and native
CAP estimates
...
Version 2.5 preserves and audits the incremental relation in
...
2,450-bit tapes (all CAP fixtures non-secure and zero external assertions; the
one-tree run freezes topology but not a complete assignment archive).\\
\addlinespace
...
exact inter-call wire identities; a complete assignment or backend-linked
witness for each streamed shard; the $2^{12}$-leaf shard; execution of a full
...
\begin{enumerate}[label=INV-\arabic*,leftmargin=*]
  ...
  relations, and version-2.5 2,048-leaf streamed shard define the implemented
  $H_{\RBBC}$ wires unambiguously.  The CAP fixtures are explicitly non-secure
  and cannot satisfy the production import contract; a full shard assignment,
  the 4,096-leaf shard, all 18 trees, and exact parent-circuit wire identities
  are still required.
...
bounded-memory row-stream digest and zero external assertions.  The 240-row
artifact gap is no longer
an engineering blocker because the fork intentionally gives up bit-exact
compatibility.  The next component is assignment-backed production
composition:

\begin{quote}
\emph{Give the streamed 2,048-leaf shard a complete backend-linked witness
assignment and whole-shard stale-witness mutation tests.  Add the
4,096-leaf/degree-13 shard, then compose all eighteen trees and their
