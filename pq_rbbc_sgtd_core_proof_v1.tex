...
\author{Research construction draft, version 2.1}
\date{23 August 2026}
...
\paragraph{Executable refinement through version 2.1.}
...
key, and an 11,644-byte final signature~\cite{blind-uov}.  Version 2.1 retains
...
Nevertheless, version 2.1 may not assert that the final signature remains
...
constraint generator.  Version 2.1 uses
...
paper's 11,644-byte signature target; version 2.1 has not established that
condition.
...
comparator, not a version-2.1 estimate.
...
\midrule
Portable v2.1 relation & 685,571 & includes 1,152 native-overlap hardening rows\\
\bottomrule
\end{tabular}
\end{center}
...
version-2.1 signature size.  Under the same provisional assumption, cached
...
figure is a superseded historical comparator; version 2.1 already counts more
...
above and records it in the version-2.1 manifest.  Fork signature and native
CAP estimates
...
Version 2.1 preserves and audits the incremental relation in
...
\path{pq_rbbc_reference_manifest_v2_1.json}.
...
Version 2.1 preserves the proof-backend exchange layer in
...
\texttt{pq\_rbbc\_br1cs\_manifest\_v2\_1.json}.
...
CAP-to-$H_{\RBBC}$ join; and a reduced executable vector with mutation tests.\\
\addlinespace
...
exact inter-call wire identities; execution of a full production vector; a fork-specific
...
\begin{enumerate}[label=INV-\arabic*,leftmargin=*]
  ...
  \item The selected sponge and version-2.1 CAP reference define $H_{\RBBC}$
  bytes unambiguously, but are insufficient to satisfy the native import
  contract until the full 18-tree row stream and exact parent-circuit wire
  identities exist.
...
fail-closed fork contract are complete.  The 240-row artifact gap is no longer
an engineering blocker because the fork intentionally gives up bit-exact
compatibility.  The next component is native lowering of the already frozen
CAP semantics:

\begin{quote}
\emph{Lower each recorded version-2.1 CAP XOF call into native
$\mathbb F_{2^{193}}$ rows while preserving the reference call order and
domain-separated tuple framing.  Stream, rather than retain, the 18-tree trace;
bind every seed, leaf commitment, tape, correction, consistency value,
canonical commitment byte, and $H_{\RBBC}$ input/output through explicit wire
identities.  First reproduce the reduced frozen vector with zero callbacks,
then run the opt-in production vector and freeze its commitment and row-stream
digests.  The parent archive must reject changed $m$, $r$, $\rho$, $c_r$, or
hash output and reach zero external assertions.  Only then prove unique-mask
and straightline extraction for the fork, revalidate blindness and one-more
