...
\author{Research construction draft, version 2.8}
\date{25 August 2026}
...
The archive records one named external assertion for the full 18-tree native
row stream and exact $H_{\RBBC}$ wire join.  Direct reference evaluation and
the canonical byte join exist, but the archive cannot yet evaluate those
operations as one native witness-independent row stream.
...
$\mathbb F_{2^{193}}$ materialization of the full 18-tree CAP row stream,
shared global transcript tail, exact inter-call wire identities across all
trees, and complete assignment replay; a fork-specific
...
\begin{enumerate}[label=INV-\arabic*,leftmargin=*]
  ...
  and cannot satisfy the production import contract; the exact shared global
  tail, cross-segment native rows, complete 18-tree assignment replay, and
...
tree.  Each has a frozen bounded-memory row-stream digest, a complete
  ...
  corrections, commitment, and request transcript.  The 240-row
...
\begin{quote}
\emph{Lower the canonical eighteen-tree schedule into exact native segments
with one shared global transcript tail, freeze every cross-segment wire, and
replay the complete assignment with zero failures.  Then join the resulting
...
The companion file
\texttt{PQ\_RBBC\_CRYPTO\_CORE\_ROADMAP\_v2\_8.md} records the ordered gates:
(R1) exact 18-tree native global-tail lowering and segmented assignment replay;
