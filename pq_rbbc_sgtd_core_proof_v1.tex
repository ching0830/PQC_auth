...
\author{Research construction draft, version 1.3}
\date{23 August 2026}
...
signature.  Backend simulation extractability and the threshold implementation
...
circuit is counted in Section~\ref{sec:circuit-audit}.  The resulting proof-size
and runtime figures remain paper-anchored estimates rather than implementation
benchmarks.  The numerical Niederreiter set is a size anchor, not a deployment
approval.
...
\addlinespace
Concretely instantiated & Blind-UOV-Is/Shorter costing profile; five-block
augmented relation; exact Niederreiter--SHAKE--KMAC trace relation;
information-theoretic unique opening; 684,419 added nonlinear constraints; and
a 4,140-byte online-ticket target.\\
\addlinespace
Not yet instantiated & A post-quantum zero-knowledge and
simulation-extractable implementation of $\Piiss$, peak prover memory, robust
threshold-share transcript, implementation benchmarks, and a reviewed response
to the August 2026 McEliece cryptanalysis.\\
\addlinespace
...
The relation, unique opening, fixed layout, and circuit audit are now complete.
The next step is an executable witness/constraint implementation plus a proof
backend qualification:

\begin{quote}
\emph{Implement the five-block witness builder and R1CS generator, test every
accept/reject invariant, and benchmark peak memory.  In parallel, either prove
that the chosen hash-based NIZK is post-quantum zero knowledge and
simulation-extractable in the required model, or revise the GCCA reduction to
the exact extractability property it actually provides.}
\end{quote}
