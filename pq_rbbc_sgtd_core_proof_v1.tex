...
\author{Research construction draft, version 2.3}
\date{24 August 2026}
...
\paragraph{Executable refinement through version 2.3.}
...
still keeps one external assertion because its 2,048-bit polynomial hash and
full tree topology are not yet native.
...
key, and an 11,644-byte final signature~\cite{blind-uov}.  Version 2.3 retains
...
Nevertheless, version 2.3 may not assert that the final signature remains
...
constraint generator.  Version 2.3 uses
...
paper's 11,644-byte signature target; version 2.3 has not established that
condition.
...
comparator, not a version-2.3 estimate.
...
\midrule
Portable v2.3 relation & 685,571 & includes 1,152 native-overlap hardening rows\\
\bottomrule
\end{tabular}
\end{center}
...
version-2.3 signature size.  Under the same provisional assumption, cached
...
figure is a superseded historical comparator; version 2.3 already counts more
...
above and records it in the version-2.3 manifest.  Fork signature and native
CAP estimates
...
Version 2.3 preserves and audits the incremental relation in
...
\texttt{pq\_rbbc\_br1cs\_manifest\_v2\_2.json}.
...
4,858-row standalone 2,450-bit tape trace; and a 113,802-row extended CAP trace
whose eight tapes have production width (all non-secure, witness-independent,
and zero external assertions).\\
\addlinespace
...
2,048-bit multi-coefficient consistency polynomial rows; a fork-specific
...
\begin{enumerate}[label=INV-\arabic*,leftmargin=*]
  ...
  native trace, and version-2.3 extended multi-squeeze trace define and test
  $H_{\RBBC}$ wires unambiguously.  The reduced and extended profiles are
  explicitly non-secure and cannot satisfy the production import contract;
  native multi-coefficient hashing, the full 18-tree row stream, and exact
...
assertions.  Generic native multi-squeeze, a frozen 2,450-bit tape, and an
extended CAP integration are complete as well.  The 240-row artifact gap is no longer
an engineering blocker because the fork intentionally gives up bit-exact
compatibility.  The next component is the part of production algebra absent
from the 64-bit reduced fixture:

\begin{quote}
\emph{Implement the production multi-coefficient polynomial hash over
$\mathbb F_{2^{193}}$ with explicit native multiplication rows.  Freeze a
standalone multiplication vector, then evaluate the 11-coefficient encoding of
the 2,048-bit witness at both transcript-derived points using Horner form.
Exercise the new polynomial relation first in a small fixture with more than
193 witness bits, then combine it with the completed 2,450-bit multi-squeeze
path and stream one full production tree shard without retaining the entire
JSON trace.
