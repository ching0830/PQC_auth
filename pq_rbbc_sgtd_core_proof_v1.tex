...
\author{Research construction draft, version 2.7}
\date{24 August 2026}
...
a 5,378-byte hidden commitment, and an exact estimate of 389,974 selected
...
ABI that accepts only the canonical 5,378-byte production-profile CAP encoding
...
have not yet been composed under one transcript.
...
The canonical commitment encoding is exactly 5,378 bytes.  It contains the
...
The full 18-tree production execution is deliberately opt-in and has not been
run for this release.  Version 2.6 runs and assignment-verifies one
2,048-leaf tree shape, but no 18-tree native row stream or parent inter-call
wire-identity proof has been produced.  Consequently the parent archive still has one named
external assertion.  Version 2.1 establishes reference semantics and the exact
byte ABI; it does not establish native circuit closure, CAP extraction,
fork-specific blindness, one-more unforgeability, or the paper's signature
size.
...
comparator, not a version-2.6 fork estimate.
...
\midrule
Portable v2.7 relation & 685,571 & includes 1,152 native-overlap hardening rows\\
\bottomrule
\end{tabular}
\end{center}
...
version-2.6 signature size.  Under the same provisional assumption, cached
...
figure is a superseded historical comparator; version 2.6 already counts more
...
above and records it in the version-2.7 manifest.  Fork signature and native
CAP estimates
...
Version 2.7 preserves and audits the incremental relation in
...
a canonical 5,378-byte production commitment encoding; a strict byte-level
...
rejected stale-witness probes (all CAP fixtures non-secure and zero external
assertions).\\
\addlinespace
Not yet instantiated & A post-quantum zero-knowledge and
simulation-extractable implementation of $\Piiss$; a native
$\mathbb F_{2^{193}}$ materialization of the full 18-tree CAP row stream and
exact inter-call wire identities across all trees; execution of a full
18-tree production vector; a fork-specific
...
\begin{enumerate}[label=INV-\arabic*,leftmargin=*]
  ...
  relations, and version-2.7 assignment-backed 2,048- and 4,096-leaf streamed
  shards define
  the implemented
  $H_{\RBBC}$ wires unambiguously.  The CAP fixtures are explicitly non-secure
  and cannot satisfy the production import contract; all 18 trees, their
  cross-tree corrections, and exact parent-circuit wire identities are still
  required.
...
canonical 5,378-byte commitment, strict byte ABI, incremental relation, and
...
and zero external assertions.  The 240-row
artifact gap is no longer
an engineering blocker because the fork intentionally gives up bit-exact
compatibility.  The next component is multi-tree production composition:

\begin{quote}
\emph{Compose all eighteen trees and their
cross-tree corrections under one canonical transcript.  The final parent
archive must reject changed $m$, $r$, $\rho$, $c_r$, or hash output and reach
zero external assertions.  Then prove
...
The companion file
\texttt{PQ\_RBBC\_CRYPTO\_CORE\_ROADMAP\_v2\_7.md} records the ordered gates:
(R1) 18-tree canonical composition; (R2) parent exact wire join and zero
external assertions; (R3) fork-specific CAP, request-binding, blindness,
