...
\author{Research construction draft, version 2.2}
\date{23 August 2026}
...
\paragraph{Executable refinement through version 2.2.}
...
key, and an 11,644-byte final signature~\cite{blind-uov}.  Version 2.2 retains
...
Nevertheless, version 2.2 may not assert that the final signature remains
...
constraint generator.  Version 2.2 uses
...
paper's 11,644-byte signature target; version 2.2 has not established that
condition.
...
comparator, not a version-2.2 estimate.
...
\midrule
Portable v2.2 relation & 685,571 & includes 1,152 native-overlap hardening rows\\
\bottomrule
\end{tabular}
\end{center}
...
version-2.2 signature size.  Under the same provisional assumption, cached
...
figure is a superseded historical comparator; version 2.2 already counts more
...
above and records it in the version-2.2 manifest.  Fork signature and native
CAP estimates
...
Version 2.2 preserves and audits the incremental relation in
...
Version 2.2 preserves the proof-backend exchange layer in
...
rate/772-bit capacity sponge with 576-bit request output.  The frozen two-block
...
CAP-to-$H_{\RBBC}$ join; and an 88,282-row zero-callback reduced native trace
(non-secure, witness-independent, and zero external assertions).\\
\addlinespace
...
exact inter-call wire identities; execution of a full production vector; a
2,450-bit multi-squeeze trace and the 2,048-bit multi-coefficient consistency
polynomial rows; a fork-specific
...
\begin{enumerate}[label=INV-\arabic*,leftmargin=*]
  ...
  \item The selected sponge, version-2.1 CAP reference, and version-2.2 reduced
  native trace define and test $H_{\RBBC}$ wires unambiguously.  The reduced
  profile is explicitly non-secure and cannot satisfy the production import
  contract; the full 18-tree row stream and exact parent-circuit wire identities
  are still required.
...
assertions.  The 240-row artifact gap is no longer
...
\begin{quote}
\emph{Extend the native sponge trace from one output rate block to a generic
multi-squeeze relation and freeze a 2,450-bit tape vector, including every
squeeze-state link and partial final lane.  Implement the production
multi-coefficient polynomial hash over $\mathbb F_{2^{193}}$ with explicit
Horner multiplication rows for the 2,048-bit witness and two evaluation
points.  Exercise both primitives first in a small extended fixture, then
stream one full production tree shard without retaining the entire JSON trace.
