...
\author{Research construction draft, version 1.5}
\date{23 August 2026}
...
request equations are fixed in Section~\ref{sec:buov-normal}; the exact added
...
\begin{enumerate}[leftmargin=*]
  ...
  \item It samples $r,\rho_0$, computes
  \[
  ...
  $z_{\BUOV}\leftarrow\BUOV.\mathsf{Respond}(\mathsf{bsk},y)$.
  \item The holder computes
  \[
    \sigma\leftarrow
    \BUOV.\mathsf{Finalize}(m,c_0,y,z_{\BUOV};r,\rho_0)
  \]
  and accepts only if
...
$H_{\mathrm{ticket}}(\mathsf{Encode}(M))$.  Equation~\eqref{eq:i5} gives a
...
with CAP-binding or hash-collision probability, the message signed through
...
\begin{proof}
Extraction yields $(M,r,\rho_0,\ldots)$ satisfying
...
$\mathcal B_1,\ldots,\mathcal B_5$ such that
\begin{align*}
 \Adv^{\mathsf{CertSound}}_{\Core,\mathcal A}
 \leq{}&
 \Adv^{\mathsf{SE\text{-}AoK}}_{\Piiss,\mathcal B_1}
 +\Adv^{\mathsf{1mUF}}_{\BUOV,\mathcal B_2}
 +\Adv^{\mathsf{bind}}_{\mathsf{CAP},\mathcal B_3}\\
 &+\Adv^{\mathsf{coll}}_{H_{\BUOV},\mathcal B_4}
...
except under the remaining binding and collision bad events.  The extracted witness
...
bounds the change.  The remaining request/response views are blind-signature views on two hidden
...
record
\[
  (d_M,M,\rid,e,k_{\mathrm{hold}},r,\rho_0,\ldots)
\]
...
\begin{align*}
 R_{\mathsf{aug}}(&\pp,\ctx,\mathsf{sid},\rid,c_0,y;\\
                  &M,r,\rho_0,k_{\mathrm{hold}},e)=1
 \quad\Longleftrightarrow\quad
 R_{\mathsf{issue}}(x;w)=1,
\end{align*}
where $\beta=(c_0,y)$ and Equation~\eqref{eq:i3} expands to
...
\begin{align*}
 \mathsf{pub}_{\mathsf{aug}}
   &= (\ctx,\mathsf{sid},\rid,\mathsf{tpk},\mathsf{bvk},c_0,y),\\
 \mathsf{wit}_{\mathsf{aug}}
   &= (\sn,k_{\mathrm{hold}},e,r,\rho_0).
\end{align*}
...
honest-protocol signer blindness, up to the SE-NIZK, CAP-binding, and
...
transcript is an ordinary Blind-UOV transcript, to which its blindness game applies.  The
...
\paragraph{Concretization achieved in versions 1.1--1.3.}
...
resistant.  Version 1.2 removes $c_M$: extraction already supplies $M$,
the request fixes its digest, and ticket-hash collision resistance prevents a
second payload from verifying under that digest.
...
3,772-byte final signature.  Its NIST-I/Shorter TCitH parameters are
...
Consequently the 3,772-byte final signature remains unchanged, while the five
ticket/trace blocks increase only the issuance NIZK circuit.

For costing, use the paper's illustrative Shockwave/Anemoi row.  Before our
extension, Blind-UOV-Is/Shorter is reported as approximately 16 million R1CS
...
cached public key buys a 288-byte trace ciphertext.  The frozen Blind-UOV-Is
profile and exact 4,140-byte complete ticket are derived in
...
constraints added beyond the paper's approximately 16-million-constraint
Blind-UOV-Is/Shorter $\pi_1$ baseline.
...
Because the paper rounds its baseline to 16 million constraints, the combined
number is itself approximate:
\[
 N_{\pi_{\mathsf{issue}}}\approx16{,}000{,}000+684{,}419
 =16{,}684{,}419,
\]
an increase of 4.2776\%.  Applying the paper's square-root proof-size scaling
and linear prover scaling gives approximately 2.86~MB and 33.4~s.  A
conservative linear scaling of its reported verifier figure gives 2.61~s.
...
the existing well-built-request NIZK changes its constraint count by less than
5\%.  There is therefore no communication-based reason to split
...
\]
Combining this with the published 3,772-byte Blind-UOV-Is/Shorter signature
gives
\[
 |T|=|M|+|\sigma|=4{,}140\text{ bytes}
\]
...
verification key.  Relative to a 70~KB online credential, the frozen target is
about 17 times smaller.  The approximately 2.86~MB issuance NIZK is offline and
...
Version 1.4 implements the incremental relation in
...
\midrule
Allocated wire identifiers & 2,976,528\\
Linear definitions & 2,281,323\\
Linear equality assertions & 3,214\\
...
an R1CS row must report that larger total explicitly.  On the present reference environment, one honest
streaming generation took approximately 5.85 seconds.  This is Python relation
...
changed masked identity, changed holder hash, changed KMAC tag, changed ticket
digest, changed blind request, and changed common context.  All eight are
rejected.  The suite contains eight test methods and completed in approximately
53.5 seconds in the same environment.  The checked output is recorded in
\texttt{pq\_rbbc\_r1cs\_manifest\_v1\_4.json}.
...
SHAKE construction, but is not a blind signature.  It marks the native
Blind-UOV $\pi_1$/CAP boundary whose nonlinear cost remains in the paper's
baseline.  Consequently this implementation establishes that the incremental
...
Version 1.5 introduces a proof-backend exchange layer in
...
\midrule
Wires & 2,976,528\\
Public input bits & 3,712\\
Secret input bits & 7,072\\
Nonlinear R1CS rows & 684,419\\
Materialized affine-definition rows & 2,281,323\\
Linear assertion rows & 3,214\\
Total R1CS rows & 2,968,956\\
Archive size & 49,183,761 bytes\\
\bottomrule
\end{tabular}
\end{center}
The 684,419 figure is therefore a \emph{nonlinear-only} cost under the affine
elimination model used in Section~\ref{sec:circuit-audit}.  The 2,968,956 figure
...
An independent reader reconstructs and evaluates all 2,968,956 rows.  It
...
wrong-weight witness produces exactly the same 49,183,637-byte body and the
...
\texttt{pq\_rbbc\_br1cs\_manifest\_v1\_5.json}.

\paragraph{Remaining external row.}
The archive records one named external assertion for the native Blind-UOV
$\pi_1$/CAP request relation, but cannot evaluate it.  This is deliberate and
...
actual Blind-UOV constraints; deleting or treating it as automatically true is
forbidden.
...
\addlinespace
Assumed component property & Post-quantum SE-NIZK AoK, CAP binding, Blind-UOV
blindness and one-more unforgeability, threshold syndrome-decoding privacy with
robust shares, QROM one-way-to-hiding, KMAC integrity, and
...
with independent row evaluation; 684,419 nonlinear rows; 2,968,956 materialized
total rows; and a 4,140-byte online-ticket target.\\
\addlinespace
Not yet instantiated & A post-quantum zero-knowledge and
simulation-extractable implementation of $\Piiss$; native Blind-UOV circuit
integration; proof-system-specific affine elimination; import of a real Goppa
...
\begin{enumerate}[label=INV-\arabic*,leftmargin=*]
  ...
  production issuance; the Blind-UOV marker must be replaced, never ignored.
\end{enumerate}

\section{Next proof step}

The relation, unique opening, fixed layout, circuit audit, executable witness
builder, rejection tests, and portable R1CS lowering are now complete.  The
next step is native Blind-UOV composition and proof-backend qualification:

\begin{quote}
\emph{Replace the external marker with the native Blind-UOV $\pi_1$/CAP
constraints and prove that its message input is exactly the 256-bit digest
produced by $\mathcal C_{\mathsf{ticketHash}}$.  Then select a concrete backend,
