...
\author{Research construction draft, version 2.9}
\date{25 August 2026}
...
six rejected stale-witness probes.\\
\addlinespace
...
\begin{enumerate}[label=INV-\arabic*,leftmargin=*]
  ...
  fixes and completely replays the exact shared global tail.  The eighteen
  producer segments, cross-segment wire identities, complete composition
  replay, and parent-circuit wire identities are still required.
...
tree.  Each has a frozen bounded-memory row-stream digest, a complete
  ...
  zero failures, and rejects six selected stale witnesses.  The 240-row
...
\begin{quote}
\emph{Generate the eighteen position-specific tree-producer segments without
duplicating the now-closed shared global transcript tail, freeze every
port-to-wire relocation, and replay the complete composition with zero
failures.  Then join the resulting
...
The companion file
\texttt{PQ\_RBBC\_CRYPTO\_CORE\_ROADMAP\_v2\_9.md} records the ordered gates:
(R1b) exact tree-producer segmentation, wire relocation, and full assignment replay;
