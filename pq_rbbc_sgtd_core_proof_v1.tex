...
\author{Research construction draft, version 1.4}
\date{23 August 2026}
...
full-circuit positive and negative tests; 684,419 added nonlinear constraints;
and a 4,140-byte online-ticket target.\\
\addlinespace
...
integration; backend matrix lowering; import of a real Goppa key; peak prover
memory; robust threshold-share transcript; implementation benchmarks; and a
reviewed response to the August 2026 McEliece cryptanalysis.\\
\addlinespace
...
builder, and rejection tests are now complete.  The next step is native
composition and proof-backend qualification:

\begin{quote}
\emph{Lower the streaming relation to the selected backend, replace the test
Blind-UOV adapter with the native $\pi_1$/CAP constraints, and prove that the
composed $\Piiss$ is post-quantum zero knowledge and simulation-extractable in
the model required by the GCCA reduction.  Then import a canonical Goppa public
key and measure prover time and peak memory.}
\end{quote}
