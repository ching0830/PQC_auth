...
\author{Research construction draft, version 2.0}
\date{23 August 2026}
...
claim.  CAP.Commit, GGM, fork-specific security proof, and fresh benchmarking
remain open and production stays fail-closed.
\end{abstract}
...
PQ-RBBC-Anemoi-193/336 sponge profile.  CAP commitment internals are not yet
instantiated.  The Blind-UOV paper is now a framework and size comparator, not
a bit-exact implementation claim.  This mixed choice retains standardized
...
\paragraph{Executable refinement in version 2.0.}
...
Version 2.0 replaces the test hash-image computation with the forked Anemoi
sponge but retains a test-only CAP commitment.  Thus the executable adapter
checks the selected $H_{\RBBC}$ primitive, while the production CAP and exact
wire join remain external.
...
key, and an 11,644-byte final signature~\cite{blind-uov}.  Version 2.0 retains
...
Nevertheless, version 2.0 may no longer assert that the final signature remains
...
constraint generator.  Version 2.0 uses
...
This closes the permutation-to-hash engineering layer, not the CAP layer.  The
48-byte commitment in the executable vector is produced by a labelled SHAKE
test fixture.  It is not a proposed production CAP encoding.  Consequently the
parent issuance archive still has one named external assertion and the
``message commitment hash'' component of the import contract remains
incomplete until real CAP output wires are joined to this sponge.
...
paper's 11,644-byte signature target; version 2.0 has not established that
condition.
...
comparator, not a version-2.0 estimate.
...
\midrule
Portable v2.0 relation & 685,571 & includes 1,152 native-overlap hardening rows\\
\bottomrule
\end{tabular}
\end{center}
...
version-2.0 signature size.  Under the same provisional assumption, cached
...
figure is likewise historical and provisional; if retained, it is offline and
...
above and records it in the version-2.0 manifest.  Fork signature and native
CAP estimates
...
Version 2.0 preserves and audits the incremental relation in
...
\path{pq_rbbc_reference_manifest_v2_0.json}.
...
\texttt{TestPQRBBC336Adapter} uses the real version-2.0 Anemoi sponge for
$H_{\RBBC}$ but derives a 48-byte commitment with a labelled SHAKE test
fixture.  It is not a blind signature or production CAP.  It marks one native
PQ-RBBC-BUOV-336 CAP.Commit-plus-hash boundary.  The 576 public $y$ bits and
...
Version 2.0 preserves the proof-backend exchange layer in
...
\texttt{pq\_rbbc\_br1cs\_manifest\_v2\_0.json}.

\paragraph{Remaining external rows.}
The archive records one named external assertion for
$h_{\RBBC}=H_{\RBBC}(m,\mathsf{CAP.Commit}(r;\rho))$, but cannot yet evaluate
the production CAP part or connect its output wires to the implemented sponge.
...
hash trace has 2,238 rows and 2,235 wires.\\
\addlinespace
Not yet instantiated & A post-quantum zero-knowledge and
simulation-extractable implementation of $\Piiss$; a native
$\mathbb F_{2^{193}}$ forked CAP.Commit circuit and exact join to the sponge;
the remaining GGM, consistency, and Fiat--Shamir components; a fork-specific
...
\begin{enumerate}[label=INV-\arabic*,leftmargin=*]
  ...
  \item The version-2.0 sponge is the selected $H_{\RBBC}$ primitive, but is
  insufficient to satisfy the native import contract until all seven complete
  CAP/GGM/transcript components and exact parent-circuit wire identities exist.
...
$H_{\RBBC}$ output, native hash rows, hidden-state adapter, incremental relation,
and fail-closed fork contract are now complete.  The 240-row artifact gap is no
longer a blocker because version 2.0 intentionally gives up bit-exact
compatibility.  The next component is the production CAP commitment and its
exact join to the implemented request hash:

\begin{quote}
\emph{Freeze the forked CAP witness and randomness serialization, beginning
with the 2,048-bit $(r,x)$ witness shape and the 18-tree level-III topology used
as a design source.  Assign distinct version-2.0 domains to seed derivation,
seed commitment, GGM expansion, consistency checks, and Fiat--Shamir hashing.
Implement CAP.Commit as native $\mathbb F_{2^{193}}$ rows, expose a canonical
commitment byte encoding, and feed those exact output bits into the already
implemented $H_{\RBBC}(m,c_r)$ sponge.  The parent archive must reject changed
$m$, $r$, $\rho$, $c_r$, or hash output without a callback.  Continue with the
remaining six CAP/GGM/transcript components until the external-assertion count
is zero.  Then prove unique-mask and straightline-extraction for the fork,
revalidate blindness and one-more unforgeability, benchmark the actual
signature, and qualify a post-quantum zero-knowledge and
simulation-extractable backend.}
\end{quote}
