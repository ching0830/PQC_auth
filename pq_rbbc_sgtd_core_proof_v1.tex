...
\author{Research construction draft, version 1.7}
\date{23 August 2026}
...
after the exact 684,419-row increment below, the total is about 37.26 million.
...
Version 1.7 extends the incremental relation in
...
\midrule
Allocated wire identifiers & 2,976,848\\
...
\path{pq_rbbc_reference_manifest_v1_7.json}.
...
Blind-UOV-III $\pi_1$/CAP boundary whose nonlinear cost remains in the
paper-anchored baseline.  The 576 public $y$ bits are serialized as circuit
inputs, while $m,r,\rho,c_r$ remain hidden.  Consequently this implementation establishes that the incremental
...
Version 1.7 uses the proof-backend exchange layer in
...
wrong-weight witness produces exactly the same 49,184,363-byte body and the
...
\texttt{pq\_rbbc\_br1cs\_manifest\_v1\_7.json}.

\paragraph{Remaining external rows.}
The archive records one named external assertion for the native Blind-UOV-III
...
materialized total rows; a 72-byte issuance request excluding proof; and a
12,012-byte online-ticket target.\\
\addlinespace
Not yet instantiated & A post-quantum zero-knowledge and
simulation-extractable implementation of $\Piiss$; a native Blind-UOV-III
circuit import and validation of the CAP properties used by
...
\begin{quote}
\emph{First replace the single external marker with the native
Blind-UOV-III $\pi_1$/CAP constraints and verify that its hidden message input
is exactly the 256-bit digest produced by
$\mathcal C_{\mathsf{ticketHash}}$.  Validate the straightline-extraction,
unique-mask, commit-before-hash, and domain-separation conditions used in
Theorem~\ref{thm:j-qrom}.  Then select a concrete backend,
