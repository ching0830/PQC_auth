...
\author{Research construction draft, version 2.4}
\date{24 August 2026}
...
constraint generator.  Version 2.4 uses
...
The full production execution is deliberately opt-in and has not been run for
this release.  No 18-tree native row stream or inter-call wire-identity proof
has been produced.  Consequently the parent archive still has one named
...
comparator, not a version-2.4 estimate.
...
\midrule
Portable v2.4 relation & 685,571 & includes 1,152 native-overlap hardening rows\\
\bottomrule
\end{tabular}
\end{center}
...
version-2.4 signature size.  Under the same provisional assumption, cached
...
figure is a superseded historical comparator; version 2.4 already counts more
...
above and records it in the version-2.4 manifest.  Fork signature and native
CAP estimates
...
Version 2.4 preserves and audits the incremental relation in
...
\texttt{pq\_rbbc\_br1cs\_manifest\_v2\_4.json}.
...
a 2,845-row production-width 11-coefficient Horner trace; and a 125,401-row
two-coefficient CAP/Horner integration whose eight tapes have production width
(all CAP fixtures non-secure, witness-independent, and zero external
assertions).\\
\addlinespace
...
exact inter-call wire identities; execution of a full production vector; the
integration of the closed 2,048-bit/11-coefficient Horner gadget across all
production symbolic mask slices; a fork-specific
...
\begin{enumerate}[label=INV-\arabic*,leftmargin=*]
  ...
  native trace, version-2.3 extended multi-squeeze trace, and version-2.4
  Horner relations define and test $H_{\RBBC}$ wires unambiguously.  The CAP
  fixtures are explicitly non-secure and cannot satisfy the production import
  contract; full 11-coefficient symbolic-mask integration, the 18-tree row
  stream, and exact parent-circuit wire identities are still required.
...
well.  The 240-row artifact gap is no longer
an engineering blocker because the fork intentionally gives up bit-exact
compatibility.  The next component is the production-scale composition absent
from the small fixtures:

\begin{quote}
\emph{Instantiate the closed 11-coefficient Horner relation for the full
2,048-bit CAP witness and every symbolic extension-mask slice in one production
tree shard.  Combine it with the completed 2,450-bit multi-squeeze path and
stream the shard without retaining the entire JSON trace.
Only after its commitment and row-stream digests are stable should the 18-tree
run begin.  The final parent archive must reject changed $m$, $r$, $\rho$,
$c_r$, or hash output and reach zero external assertions.  Then prove
