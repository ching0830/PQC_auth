...
\author{Research construction draft, version 1.2}
\date{23 August 2026}
...
the decoded error, removes a redundant ticket commitment, and gives exact
online byte counts.  The exact CAP circuit, frozen parameters, and
implementation proof remain separate obligations.
\end{abstract}
...
entire ticket relation into the selected post-quantum CAP circuit and performing
concrete gate accounting remain implementation tasks.  The numerical
Niederreiter sets in this draft are size anchors, not a deployment approval.
...
\[
 ...
 \qquad C=\mathsf{Ntr.Enc}(\mathsf{tpk},\mathsf{ci},h;\rid,\sn,e).
\]
...
\begin{align*}
 x&=(\pp,\mathsf{ci},\mathsf{sid},\rid,\beta),\\
...
\begin{align}
 ...
 R_{\mathsf{enc}}(\mathsf{tpk},M.C,
   \mathsf{Encode}(\mathsf{ci},M.\sn,M.h);\rid\|M.\sn,e)&=1.
   \tag{I5}\label{eq:i5}
\end{align}
...
\begin{enumerate}[leftmargin=*]
  \item The holder samples $\sn,k_{\mathrm{hold}},e$, computes
  $h=H_{\mathrm{hold}}(k_{\mathrm{hold}})$ and
  $C=\mathsf{Ntr.Enc}(\mathsf{tpk},\mathsf{ci},h;\rid,\sn,e)$
...
\begin{align*}
 R_{\mathsf{aug}}(&\pp,\mathsf{ci},\mathsf{sid},\rid,c_0,y;\\
...
\begin{align*}
 \mathsf{pub}_{\mathsf{aug}}
   &= (\mathsf{ci},\mathsf{sid},\rid,\mathsf{tpk},\mathsf{bvk},c_0,y),\\
...
\begin{align*}
 ...
 \mathcal C_{\mathsf{trace}} &: R_{\mathsf{Ntr}}
   (\mathsf{tpk},C,\mathsf{ci},h;\rid,\sn,e)=1.
\end{align*}
...
\paragraph{Concretization achieved in versions 1.1--1.2.}
...
resistant.  Version 1.2 also removes $c_M$: extraction already supplies $M$,
...
\begin{align}
 ...
 \mathsf{ad}&=\mathsf{Encode}(\mathsf{ci},\sn,h), &&\tag{N2}\label{eq:n2}\\
 Z&=\mathsf{SHAKE256}(
   \mathtt{PQ\mbox{-}RBBC/KDF}\|\mathsf{Encode}(e,s,\mathsf{ci}),640),
   &&\tag{N3}\label{eq:n3}\\
 (K_{\mathrm{mac}},P)&=Z,
   &|K_{\mathrm{mac}}|&=32,\quad |P|=48, \tag{N4}\label{eq:n4}\\
 c_\mu&=\mu\oplus P, &&\tag{N5}\label{eq:n5}\\
 \gamma&=\mathsf{KMAC256}_{K_{\mathrm{mac}}}
 (\mathtt{PQ\mbox{-}RBBC/TAG}\|s\|c_\mu\|\mathsf{ad},256),
 &&\tag{N6}\label{eq:n6}\\
...
\[
 R_{\mathsf{Ntr}}(\mathsf{tpk},C,\mathsf{ci},h;
                    \rid,\sn,e)=1
\]
...
\[
 ...
 &R_{\mathsf{enc}}(\mathsf{tpk},C,
     \mathsf{Encode}(\mathsf{ci},\sn,h);\rid\|\sn,\rho_{\mathrm{tr}})=1\\
 &\qquad\Longleftrightarrow\quad
 R_{\mathsf{Ntr}}(\mathsf{tpk},C,\mathsf{ci},h;\rid,\sn,e)=1,
...
input $(e,s,\mathsf{ci})$ by a uniform string.  A distinguisher for this step
...
cached public key buys a 288-byte trace ciphertext.  With a 5--8~KB Blind-UOV
signature, 16-byte serial, 32-byte holder hash, and roughly 32--64 bytes of
compact common identifiers/digests, the online ticket is approximately
5.4--8.5~KB.  This is an
engineering estimate, not a benchmark, but it is roughly 8--13 times smaller
than a 70~KB design.
...
\addlinespace
Concretely instantiated & Blind-UOV request/message binding; five-block augmented
relation; exact Niederreiter--SHAKE--KMAC trace relation; information-theoretic
unique opening; trace correctness; and online trace byte counts.\\
\addlinespace
Not yet instantiated & Exact MPC-in-the-Head/CAP gate counts and parameters,
robust threshold share transcript, implementation benchmarks, and a reviewed
response to the August 2026 McEliece cryptanalysis.\\
\addlinespace
...
The next step is not satellite integration.  The trace relation and its unique
opening are now fixed.  The remaining concrete step is a circuit audit:

\begin{quote}
\emph{Freeze one Blind-UOV revision, arithmetize all five blocks---shape,
ticket hash, Blind-UOV mask, holder hash, and the exact (N1)--(N7) trace
relation---in its MPC-in-the-Head/CAP backend, then count nonlinear gates, hash
permutations, proof bytes, prover memory, and issuer verification cost.}
\end{quote}

This step will decide whether one monolithic proof is viable or whether
$R_{\mathsf{aug}}$ must be split into compositionally linked proofs.  The
Blind-UOV parameters may be frozen for that experiment; the Niederreiter
deployment parameter MUST remain provisional under
Remark~\ref{rem:mceliece-hold}.  The satellite AKE remains out of scope until
this core closes.
